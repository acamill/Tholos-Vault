...
pub mod parea_vault {
    ...
    //Transfer SOL out of the account
    pub fn transfer_sol(ctx: Context<Auth>, dest: Pubkey, amount: u64) -> Result<()> {
        let multisig = ctx.accounts.multisig.to_account_info().key;
        solana_program::system_instruction::transfer(multisig, &dest, amount);
        Ok(())
    }
...
pub struct Transaction {
    ...
    did_execute: bool,
    //nonce associated with the multisig account
    tx_nonce: u64,
